...
interface AnnouncerProfileProps {
...
export default function AnnouncerProfile({ announcer, className }: AnnouncerProfileProps) {
  ...
  const openAnimalLightbox = (animalId: string, photoIndex: number) => {
    setLightboxAnimalId(animalId);
    setLightboxIndex(photoIndex);
  };

  const closeAnimalLightbox = () => {
  ...
  return (
    <section className={cn("space-y-6", className)}>
                              ...
                              {animal.galleryPhotos?.map((photo, photoIndex) => (
                                <button
                                  ...
                                  onClick={(e) => {
                                    ...
                                    openAnimalLightbox(animalId, idx);
                                  }}
                                  className="relative flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden shadow-md hover:shadow-lg hover:scale-105 transition-all ring-2 ring-white"
                                >
                                  <Image
      ...
      <ImageLightbox
        ...
        altPrefix={`Photo de ${announcer.firstName}`}
      />
