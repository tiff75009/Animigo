...
export default function DashboardFaqArticlePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <Link href="/dashboard/aide" className="hover:text-primary transition-colors">
          Centre d'aide
        </Link>
        <span>/</span>
        {category && (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-white rounded-2xl">
          ...
          <Link href="/dashboard/aide" className="text-primary hover:underline">
        ...
        <>
              ...
              <div className="space-y-2">
                {relatedArticles.map((related: (typeof relatedArticles)[number]) => (
                  <FaqArticleCard
                    key={related._id}
                    article={related}
                    href={`/dashboard/aide/article/${related.slug}`}
                  />
                ))}
              </div>
            </motion.div>
          ...
          {/* Retour */}
          <Link
            href={category ? `/dashboard/aide/${category.slug}` : "/dashboard/aide"}
