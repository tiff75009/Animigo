...
import { XCircle, Calendar, Euro, Loader2 } from "lucide-react";
...
export default function MissionsRefuseesPage() {
  ...
  // Query Convex pour les missions "refused"
  const missions = useQuery(
    api.planning.missions.getMissionsByStatus,
    token ? { token, status: "refused" } : "skip"
  );
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-text-light">
              {missionsList.length} mission{missionsList.length > 1 ? "s" : ""} refusée{missionsList.length > 1 ? "s" : ""}
            </p>
          </div>
        </div>
      </motion.div>
