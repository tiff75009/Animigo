...
export const getInvoiceFullData = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      clientAddress,
      clientStreet: recipientProfile?.location || "",
      clientPostalCode: recipientProfile?.postalCode || "",
      clientCity: recipientProfile?.city || "",
      announcerName: emitter ? `${emitter.firstName} ${emitter.lastName}` : "",
      announcerEmail: emitter?.email || "",
      announcerPhone: emitter?.phone || "",
      announcerAddress,
      announcerStreet: emitterProfile?.location || "",
      announcerPostalCode: emitterProfile?.postalCode || "",
      ...
      sapMention,
      sapApprovalNumber,
      companyLogoUrl: emitterProfile?.companyLogoUrl || "",
    };
  },
});
